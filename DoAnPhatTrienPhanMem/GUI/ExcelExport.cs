...
namespace GUI
{
    public class ExcelExport
    {
        ...
        public const string T_DanhMucKhoa = "HoaDon";
        #endregion
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer,DateTime dateVao,DateTime dateRa, double thanhTien,int sl,double tienKhachDua,double tienTraLai,int soHD,int maBan,int maNV)
        {
            if (pReplacer != null)
            {
                ...
                pReplacer.Add("%TienThanhToan", ""+thanhTien);

                pReplacer.Add("%SLMon", ""+sl);

                pReplacer.Add("%TienKhachDua", ""+tienKhachDua);
                pReplacer.Add("%TienMat", ""+tienKhachDua);

                pReplacer.Add("%TienTraLai", ""+tienTraLai);

                pReplacer.Add("%SoHD", "Số HD: "+soHD);
                pReplacer.Add("%SoBan", "Mã Bàn: "+maBan);
                pReplacer.Add("%MaNV", "Mã NV: "+maNV);
                pReplacer.Add("%GioRa", "Thời Gian Ra: "+dateRa);
                pReplacer.Add("%GioVao", "Thời Gian Vào: "+dateVao);
        ...
        public bool ExportKhoa(List<tblChiTietHD> dataSource, ref string fileName, bool isPrintPreview,DateTime dateVao,DateTime dateRa, double thanhTien,int sl,double tienKhachDua,double tienTraLai,int soHD,int maBan,int maNV)
        {
            // Check if data is null
            ...
            for (int i = 1; i <= dataSource.Count; i++)
            {
                dataSource[i - 1].TenMon = dataSource[i-1].tblThucDon.TenMon;
            }
        ...
        //    replacer.Add(V_TONGSO, "100");
            BuildReplacerCurrentDate(ref replacer, dateVao,dateRa,thanhTien,sl,tienKhachDua,tienTraLai,soHD,maBan,maNV);
